...
function knightMove (start, end, visited = []) {
    ...
    if (getAlreadyVisited(end, possibleMoves)) return [start, ...visited, end];


    let shortest = null;
    console.log('possibleMoves =', possibleMoves);
    for(move of possibleMoves) {
        const result = knightMove(move, end, [...visited, start]);
        console.log('result =', result);
        if (result !== null) {
            result.push(move);
            if (shortest === null || shortest.length > result) shortest = result;
        }
    }
    return shortest;
}
...
console.log(knightMove([0,0],[0,4]))   //[[0,0],[1,2]]
